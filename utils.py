...
class Face(Organ):
    ...
    def slimface(self, rate=1.0):
        ...
        r_left  = math.sqrt((left_landmark[0] - left_landmark_down[0])*(left_landmark[0] - left_landmark_down[0])+
                            (left_landmark[1] - left_landmark_down[1])*(left_landmark[1] - left_landmark_down[1]))

        # 计算第14个点到第16个点的距离作为瘦脸距离
        r_right = math.sqrt((right_landmark[0] - right_landmark_down[0])*(right_landmark[0] - right_landmark_down[0])+
                            (right_landmark[1] - right_landmark_down[1])*(right_landmark[1] - right_landmark_down[1]))
        r_left  *= rate
        r_right *= rate

        #瘦左脸
        self.img_bgr = localTranslationWarp(self.img_bgr, left_landmark[0], left_landmark[1], endPt[0], endPt[1], r_left)
        #瘦右脸
        self.img_bgr = localTranslationWarp(self.img_bgr, right_landmark[0], right_landmark[1], endPt[0], endPt[1], r_right)
